...
function deviceMotionHandler(eventData: DeviceMotionEvent) {
  // 获取含重力的加速度
  ...
  if (acceleration && acceleration.x != null && acceleration.y != null && acceleration.z != null) {
    ...
    if (diffTime > 100) {
      ...
      if (speed > SHAKE_THRESHOLD) {
        ...
        shakeHandlerList.forEach(shake => shake.handler());
      }
...
export interface ShakeEventObject{
  id: string;
  handler: () => void;
}
const shakeHandlerList: ShakeEventObject[] = [];
let isListen = false;
...
export function removeShakeEvent(shakeObj: ShakeEventObject){
  _.remove(shakeHandlerList, shakeObj1 => shakeObj1.id === shakeObj.id);
  if(shakeHandlerList.length == 0){
    unlisten();
  }
}
